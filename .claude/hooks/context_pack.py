...
def walk_items(tickets_json):
    if isinstance(tickets_json, dict):
        for mod in tickets_json.get("modules", []):
            fpath = mod.get("file") or mod.get("module")
            for it in mod.get("items", []):
                if isinstance(it, dict):
                    yield fpath, it
    elif isinstance(tickets_json, list):
        for it in tickets_json:
            if isinstance(it, dict):
                fpath = it.get("file") or it.get("module")
                yield fpath, it


if ticket and tickets:
    for fpath, it in walk_items(tickets):
        ticket_id = it.get("id") or it.get("ticket_id")
        if ticket_id == ticket:
            target_file = fpath
            ticket_slice = it
            break
...
if not ticket_slice:
    for fpath, it in walk_items(tickets):
        ticket_id = it.get("id") or it.get("ticket_id")
        target_file, ticket, ticket_slice = fpath, ticket_id, it
        break
...
if target_file and os.path.exists(os.path.join(proj, target_file)):
    ...
    with open(os.path.join(proj, target_file), encoding="utf-8") as f:
...
pack = {
    ...
    "contracts": os.listdir(os.path.join(proj, "contracts"))
    if os.path.exists(os.path.join(proj, "contracts"))
    else [],
